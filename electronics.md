...
Jump to: [Batteries](#battery), [actuators](#actuators), [connectors](#connectors), [DIN rail](#din_rail), [dupont](#dupont), [LED Lights](#led_lights), [microcontrollers](#mcu), [pigtail](#pigtail), [sensors](#sensors), [servos](#servos), [solder](#solder), [switches](#switches), [terminals](#terminals)
...
Servos of this type include few form factors because these devices became standard through a few decades of popularity in remote-controlled cars and planes. Engineering teams like Futaba poured strong technology development into this field in such a way that expanded the embedded technology tenfold compared with a bare DC motor.  Servos are broadly defined as a motor with a feedback control loop, but this bin includes just one type of servo motor and a few variations thereof: high torque, high speed, continuous vs 180 vs 360 degrees. The cost is far lower than other servo designs and sophistication per dollar is amazing.  Expect to pay below $10 each, including metal geartrain, an analog sensor system, and sets of “servo horns” which couple to the output shaft for various lever arms or linkages. These devices operate at 5v or 6v and consume low current suitable for the ~24 gauge copper leads.  The engineering goes all the way to the failure modes, with the gear teeth on the servo horns having weaker plastic than the output gear, so the expendable portion is damaged in a crash condition while the main portion is safe.

You can learn all the fundamentals to operate a servo with a [fantastic tutorial from HowToMechatronics](https://howtomechatronics.com/how-it-works/how-servo-motors-work-how-to-control-servos-using-arduino/). Despite their origins in the hobby market where they are driven by dedicated single-purpose controllers, the servos respond to a standard pulse input with 50 hz and 20milisecond pulse width at 5 volts.  This is easily achievable with the popular low-cost MCUs and a custom-written program because the MCU pins match in their sizing and the output pins can easily fine-tune around a 20 milisecond range.  So, one could say this is the most sophisticated complete control system having a mechanical actuator under $20, or the most powerful torque-generating item that could be mated to a MCU board without supplemental power.  For any larger actuation efforts, we move out of a small bin and into individual orders and selections of actuators.
...
**Enclosed Switches**
* [limit switch ME8104](https://amzn.to/4ppvsYF) with roller 
* [limit switch ME8108](https://amzn.to/4picVgL) with roller lever
* [limit switch ME9101](https://amzn.to/3QYXxJX) with flexible spring
